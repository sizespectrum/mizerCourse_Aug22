...
The default fishing gear selects all individuals above maturity size of all species with a catchability of 1. So if we set the fishing effort to 0.3 and we have modified catchability of species in the previous tutorial to reflect final fishing mortality more accurately.

Last week we discussed that natural choice for the allometric exponents $n$ (growth exponent) and $p$ (metabolic exponent) is to take them both equal to $3/4$. That is the choice we will use. By default these exponents in multi-species models are set to different values, so we will overwrite the defaults in our `newMultispeciesParams()` command.
...
**ASTA: I mentioned this in the previous tutorial, so we could remove this part. Or we can repeat, since people don't read everything thoroughly anyway**
...
#ASTA - you mentioned that you don't want people to modify or view parameters using this approach, but instead use species_params function. Would be good to be consisten 
...
We were lucky and only had to wait for another 13.5 years. **ASTA - not for me! Second round also didn't converg for 99 years. Third time it converged after 58 years.** We'll discuss what to do when we are not lucky in the [When things don't go smoothly](#when-things-dont-go-smoothly) section.
