...
When you repeat this work in your worksheet, you can check that the data was read in correctly by clicking on `curonian_species_params` and `curonian_interaction` in the "Environment" tab. That will open the data frames in your editor window for you to inspect.
...
The default fishing gear selects all individuals above maturity size. The fishing mortality that these selected individuals experience is the product of the catchability of their species with the fishing effort. We set the fishing effort to 0.3 and we have set the catchability of the species in the previous tutorial to reflect final fishing mortality more accurately.
...
```{r}
saveParams(cur_model, "cur_model.rds")
```


## If the method fails

There are ways how the above method can fail. If that happens, there are various ways to rescue the situation. But rather than discussing such eventualities in the abstract, we will wait for course participants to run into concrete difficulties and to share their example with us. We will then use those examples to discuss the solutions.
