...
-   If your R project is located inside a directory with a very long path name, you might get an error messge because R will not be able to access the path. \
    The error message was 'Warning in gzfile(file, "wb"): cannot open the compressed file 'C:/..... " \
    **Solution**: make sure you create your R git project in a higher directory with a shorter path

-   For some users, Github asks for username and password (or personal access token) every time you want to push your commits to Github. This can happen if you Git version is too old. \
