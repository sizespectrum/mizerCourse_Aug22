...
Let's start by exploring what happens if we change fishing in the future. To do this we set up a scenario, where the model starts with the last time step of the fished scenario. Let's pick up the the model output we saved from the end of the last tutorial.
...
Let's start a new simulation that begins with the logistic effort from 2020 and projects forward for 50 years. We will apply a linear increase in effort for toothfish to a target value (here assumed for simplicity to be F=0.5).

To do this we need to work with the effort array (time x gear) to enable changes in effort through time, for this scenario.

```{r, code_folding=T, echo = FALSE}
sim_status_quo<-project(params,initial_n = simf3@n["2020",,], initial_n_pp = simf3@n_pp["2020",],effort=current_effort,t_max = 50)
proj_effort <- matrix(current_effort, nrow = 50, ncol = 1, byrow = TRUE)
dimnames(proj_effort) <- list(time = 2021:2070,
                              gear = unique(gear_params(params)$gear))
# if we are working in the same units as the effort above
# we can change the F below to scale effort to be consistent with a value of F=0.2
# E= catchability/F 
# otherwise if you assume F= Q*E, and set E to 1, you can use F=0.2 directly for the
target<-0.2/gear_params(params)$catchability[1]
select_gear="longline"
# reach target by 10 years
proj_effort[1:10,select_gear] <- 
    seq(from = proj_effort[1], to = target, length = 10)
# then hold at target
proj_effort[11:50, select_gear] <- target
# check it
#plot(proj_effort,typ="l")
...
How has this affected the catches and the biomass of other species in the system?

```{r}
plotYield(sim_scen)
# plot change in biomass relative to 2020 values
...
We can see that when we increase fishing on toothfish it slightly decreases the biomass of this species but also affects the biomass of other species in the community.
