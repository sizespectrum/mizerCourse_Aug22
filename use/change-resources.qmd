...
Now change the lambda parameter, first for plankton and then for benthos. For plankton decrease lambda from 2.05 to 2.00. For benthos decrease lambda from 1.9 to 1.8. Project for 50 years, save results and commit.
...
Gustav to add a paper where extra juvenile mortality makes a big difference with r_pp
...
1\) You can use a simple Arrhenius equation of exponentially increasing physiological rates. See examples here and here. Explain how to update rates with preferred rates.

2\) You can use therMizer addon, which aims to account for dome-shaped species responses to temperature - increased rates up to a certain temperature and decreases after that. Here you can allow for different temperature sensitivities among species. Change with temperature in time demonstratio.

3\) Of course climate change will also affect the composition of species, because with warming new species will enter the ecosystem. This is an important aspect to address, but has not been thoroughly explored yet. Mizer allows you to add new species into ecosystem, and you can find an example in the mizerHowTo package and on this link. However, it is not necessarily clear how we tune parameters of new species for which we don't yet have data (in that ecosystem at least), so exploring climate heating driven species composition change is not straightforward.
