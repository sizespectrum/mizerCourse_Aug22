...
-   The parameter **`erepro`** describes the efficiency with which the energy that a species invests into reproduction is converted to eggs. Multiplying $E_R$ by `erepro` gives the quantity that we called $R_{di}$: the rate of egg production. We also sometimes refer to $R_{di}$ as the *"density-independent reproduction rate"*. The efficiency `erepro` can be quite small because a lot of the energy expended on reproduction goes not into the production of eggs but in associated processes, like for example migration to the spawning grounds.
...
The black dot in the diagram is meant to represent the steady-state values for $E_R$ and $R_{dd}$. The solid curves represents how the density-dependent reproduction rate $R_{dd}$ depends on the rate $E_R$. Both the solid blue line and the solid black line go through the black dot, which means that they result in the same steady-state reproduction. They differ by the choice of the maximum reproduction rate $R_{max}$ (the height of the dotted line) and the reproductive efficiency `erepro` (the slope of the dashed line representing $R_{di}$).
...
```{r}
species_params(cm) |> select(resilience)
```

These values are derived from single species models with very different assumptions to those of ours, so they can only be used as very general guides. Alternatively, we can use [Ken Andersen's book](https://books.google.dk/books?hl=en&lr=lang_en&id=M-6YDwAAQBAJ&oi=fnd&pg=PP1&ots=XbQp07hJ2w&sig=59ugnqv17dH-KsbV2LVRB8rB9CM&redir_esc=y#v=onepage&q&f=false) and expectations for species with different asymptotic sizes, as estimated from trait-based models (where all species parameters are determined in terms of the asymptotic size) and when all species are fished with 50% selectivity at 5% of their asymptotic size. The panels below represents species with different asymptotic sizes: 333 g (top), 10 g (bottom left), and 10 kg (bottom right). The lines show yield (solid black lines), yield per recruit (dashed lines), spawning stock biomass (dark gray lines), and recruitment (light gray lines), all scaled by their maximum value. We will not deal with yield per recruit, so ignore the dashed lines. This figure shows that highest yields are expected at fishing mortality of about 0.3-0.5/year.
...
Because we want all species to have the same value, we choose a value that is larger than those required. So we choose 0.001.
...
This tells us that most species can produce large amounts of eggs, but the actual reproduction is strongly capped by the `R_max` parameter.

## Tuning resilience to fishing

Let's explore how changing reproduction levels will change our species' resiliences to fishing.
...
To set reproduction level to reasonable levels and get reasonable response to fishing, we will apply a range of fishing mortalities to one species at a time and check how their yields, spawning-stock biomasses (SSB) and reproduction rate (RDD) will change.

For that purpose we project the model with a range of different fishing mortalities. For each fishing mortality we run the projection until the system has settled down to a new steady state. Then we calculates the yield, the spawning stock biomass SSB and the reproduction rate RDD in that steady state. After doing that for all fishing mortalities we can plot all the results in a graph. The `plotFCurves()` function does all that for us:

```{r}
#| message: false 
plotFCurves(cm_generic_gear, species = "smallfish", F_max = 2, no_steps = 20)
```

We refer to the yield achieved in the steady state as the *sustainable yield* because because it can be sustained indefinitely. The sustainable yield for this speices has a maximum at a fishing mortality of about 0.8. We refer to this value as $F_{MSY}$. The small fish have a high resilience to fishing. This is as expected, so we do not need to make changes to the resource level of "smallfish".

Next we look at ruffe. This is classified as a medium resilience species, so we would like the F_MSY to be between 0.1 and 0.5.

```{r}
#| message: false 
plotFCurves(cm_generic_gear, species = "ruffe", F_max = 1, no_steps = 20)
```

We see that the F_MSY is at about 0.4, so again we have no reason to change the reproduction level. We encourage you to investigate the yield curves of the other medium resilience species in a similar way.

::: callout-note
Notice that for some fishing mortalities, `plotFCurves()` gives a warning that convergence was not reached within the 99 years for which we ran the projection. This means that species didn't settle into a new stable state with a new fishing level and thus the result for this specific fishing level is only approximate. But approximate values are good enough for our purpose. Often we find that it is the more extreme values of F where we do not get convergence.
:::

We jump to the first low resilience species in our model: perch. As a low resilience species it should have an F_MSY below 0.25.

```{r}
#| message: false 
plotFCurves(cm_generic_gear, species = "perch", F_max = 1, no_steps = 20)
```

The graph shows that the F_MSY is closer to 0.7. So we have reason to reduce the reproduction level for perch to make it less resilient. Its current reproduction level is

```{r}
getReproductionLevel(cm_generic_gear)["perch"]
```

We do not know how much we need to reduce the reproduction level to get a sufficiently low F_MSY. We just try some values. Let's try 0.3.

```{r}
#| message: false 
# First we save current reproduction level into a vector 
rep_level <- getReproductionLevel(cm_generic_gear)
# the we replace our species reproduction level with a new value 
rep_level["perch"] <- 0.3
...
With that reproduction level of 0.3 for perch, its F_MSY is just below 0.5. So still too high for a low resilience species. But before we try to reduce the reproduction level further, let's move on to the next species, pikeperch.

```{r}
#| message: false 
plotFCurves(cm_generic_gear, species = "pikeperch", F_max = 1, no_steps = 20)
```

Its F_MSY is also too high. So we also reduce the reproduction level of pikeperch.
...
The new F_MSY is at the upper end of what we would expect for a low-resilience species, but we judge this to be o.k.

::: callout-note
Notice that for high values of the fishing mortality `plotFCurves()` issues a warning to let us know that pikeperch will go extinct under this fishing mortality. That is fine.
:::
...
We decided to increase the reproduction level for perch a bit again to make it slightly more resilient.
...
Let's plot the yield curve for a species that is caught commercially: breams
...
plotFCurves(cm, species = "breams", F_max = 1, no_steps = 20)
# Yield curve with generic gear
plotFCurves(cm_generic_gear, species = "breams", F_max = 1, no_steps = 20)
```
...
We can see that with the commercial gear parameters, selectivity starts much later, so we are only catching the largest individuals. Therefore the species can sustain a much higher fishing mortality for those selected individuals.

## Summary

1.  Identifying approximate reproduction level for our model species is very important, because reproduction level will determine species resilience to fishing.

2.  To determine approximate reproduction level we estimate fishing mortality that will give maximum sustainable yield ($F_{MSY}$). We use general fisheries and size ecology theory to get expected $F_{MSY}$ values for species with different life history parameters. Naturally, these are just approximate estimates, but they might be good enough for our purposes. Nevertheless, it is important to explore how our study conclusions might be affected by changes in species reproduction level.
...
4.  Species resilience to fishing is a true multi-species phenomenon. Even if we fish only one species in an ecosystem, other species will be affected by its change in biomass and will in turn affect our fished species. So it is important to explore resilience to fishing repeatedly for the same species, while we change reproduction levels of other species.
