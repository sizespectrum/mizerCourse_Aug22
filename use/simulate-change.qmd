...
We can see from the data that there have been big changes in effort and catches through time. The three plots show effort in units of number of hooks, catches are in tonnes, and th efinal plot shows how the area of the fishing activity has expanded through time.

Previously we assumed effort was 1 and we worked with catchability as the variable.  Real effort data come in all sorts of differennt units (hours or days fished, Kilowatt days, number of vessels all per unit time). The key to using this in our model is that the catchability  is the fraction of the available (that is also selected by the gear) stock caught per unit of any effort that is included.
...
relative_effort<-dat$Effort/mean(dat$Effort)

effort_time <-array(c(rep(0,53),dat$Effort_km2), dim = c(length(1950:2020),1),dimnames = list("time" = 1950:2020, "gear" = params@gear_params$gear[1]))
...
Many fisheries develop through time according to a logistic function , followed by a decline in effort if stocks drop below sustainable levels and management kicks in. Try a logistic fishing with an initial value of F_init =0 and the year at which Max Catch achieved set F to Fmsy
...
effort_time2<-effort_time
yr<-1950:2020
effort_time2[,1]<-(1.5/(1+ exp(-0.5*(yr-1990))))
plot(effort_time2,typ="l")
...
Optional :
Exercise- set up another gear or a bycatch on on other the other species - follow same effort but with a lower catchability 
