...
Let's first explore some examples of what can be done using a mizer model for the Curonian lagoon. We will load the model from our previous tutorial where we tuned reproduction level of each species and returned gear selectivity to the original Curonian lagoon gear.
...
The first option most users want to explore is to see what happens if we start fishing more or if we reduce fishing mortality. We will use our Curonian model with tuned reproduction and change total effort. We will run both models (same effort and increased effort) to steady state and compare. Whenever we are saving simulations (with time dimension) we will use sim\_ in object names, to separate them from params objects
...
```{r}
sim_cur_model_Pikeperch <- project(cur_model_fishPikeperch, effort = 0.3, t_max = 100, dt = 0.1)
plotlyBiomass(sim_cur_model_Pikeperch)
```
...
plotlyBiomass(sim_cur_model_largerMesh)
...
```{r}
plotlyBiomass(sim_cur_model_gillnet)
```
...
```{r}
sim_cur_model_gillnet <- project(cur_model_gillnet, effort = 0.3, t_max = 20, dt = 0.1)
plotlyBiomass(sim_cur_model_gillnet)
```
...
```{r}
getBiomass(sim_cur_model_gillnet)[idxFinalT(sim_cur_model_gillnet), ] / getBiomass(sim_cur_model_sameFishing)[idxFinalT(sim_cur_model_sameFishing), ]

getYield(sim_cur_model_gillnet)[idxFinalT(sim_cur_model_gillnet), ] / getYield(sim_cur_model_sameFishing)[idxFinalT(sim_cur_model_sameFishing), ]
```
...
```{r}
p_protectLargeFish <- setInitialValues(sim_cur_model_gillnet@params, sim_cur_model_gillnet)
...
Quite often managment options include slow decrease in fishing effort to allow stocks to rebuild, but fishers still catch something. We can simulate that by providing mizer with a time series of effort. Or what if we want to change effort on one species only? We need to change catchability then?
...
1.  Mizer has lots of options to explore changes in gears, effort and fishing dynamics. In this tutorial we only explored basic options and found that even with very basic changes ecosystem responses can be hard to predict and fully understand.
...
There are a lot more ways how we can explore fishing scenarios (past and future) in mizer. You can explore [mizerHowTo](https://github.com/sizespectrum/mizerHowTo) R package, which provides further tutorials about: 1) statistically fit models to time series, 2) set up logistic change in effort when only catch data is available, 3) exploring model outputs with past effort data, and so on.

You can also read more on mizer blog and webpage. For example, a blog post on gear selectivity, etc, etc (**TODO - Gustav, add links**?)
