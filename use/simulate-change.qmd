...
Quite often managment options include slow decrease in fishing effort to allow stocks to rebuild, but fishers still catch something. We can simulate that by providing mizer with a time series of effort. 
...
getEffort(sim_cur_model_sameFishing)[idxFinalT(sim_cur_model_sameFishing), ]

#We can set effort as a vector for each species 
effort_vector <- rep(0.3, length(species_params(cur_model)$species))
effort_vector

#check that sim
same_effort <- project(cur_model, effort = effort_vector, t_max = 20, dt = 0.1)
...
You can also learn more on mizer blog and webpage. For example, a blog post on gear selectivity, etc, etc (**TODO - Gustav, add links**?)
