...
The energy income for a fish comes from predation on its prey. If there is less prey, the fish consumes less and its growth rate will decrease. Let us investigate this by artificially removing some prey.
...
# Create logical vector identifying the size bins we want to change. Here w_full is a mizer vector for all size bins including community and modelled species (you can also see it as params@w_full)
...
The plot show only the resource biomass density and we see a big drop in resource in our selected size range. This reduced availability of prey in that size range will lead to a drop in the growth rate in the fish that feed in that size range. Remember, we can modify the resource range this way here, because we currently assume that resource spectrum is constant and does not change dynamically from fish feeding on it. We can see the slow-down in growth by comparing the growth rates in the original model and the new model. We will use the method that we saw at the end of the previous tutorial in the section "[Two curves in one plot](single-species-spectra.qmd#two-curves-in-one-plot)".
...
ggplot(gf, aes(x = w, y = value, colour = Model)) +
    geom_line() +
...
```{r}
bf_original <- plotSpectra(params, power = 2, return_data = TRUE)
bf_starved <- plotSpectra(params_starved, power = 2, return_data = TRUE)
```
...
We see that the predator of size 10g likes to feed on prey that is about the size of 0.1g, which is about 100 times smaller than itself. But it also feeds on other sizes, just with reduced preference. The preferred predator/prey size ratio is determined by the species parameter `beta` and the width of the feeding kernel, i.e., how fussy the predator is regarding their prey size, is determined by the species parameter `sigma`. In broader literature beta is usually called predator prey mass ratio or PPMR. Larger PPMR values mean that the predator feeds on a relatively smaller prey (larger ratio). In our model these have the values
...
```{r}
#put resource abundance values into params_pk 
initialNResource(params_pk) <- initialNResource(params_starved)
# now find a new steady state, because conditions have changed and new equilibrium numbers at size must be determined 
...
The feeding level will depend on the maximum intake, search rate and food availability. The maximum intake rate scales with body size with the exponent $n = 3/4$ and is determined by the coefficient $h$. So the maximum intake rate at size $h(w)$ is modelled as $h(w) = h w^n$. Again, if we want to modify maximum intake rate we usually change the coefficient $h$. We will do that in later tutorials. The current value of the coefficient $h$ is
...
Once we have specified the predation parameters, these parameters determine both the growth of predators but also the mortality rate of prey. So we don't have to introduce new parameters for death from predation. Of course with a one species model we cannot easily demonstrate this predation, although even in this model larger size groups are feeding on smaller size groups (so they are cannibalistic). We will explore predation more thoroughly in future tutorials.
...
In addition, mizer allows you to include other sources of [external mortality](https://sizespectrum.org/mizer/reference/setExtMort.html). This could be predation from animals that we have not included in our model, like sea birds or mammals, death from old age (senescent death) and so on. Mizer allows setting of background death with `setExtMort()`. Remember, that by default in a simple single species model we don't include these mortality values.
...
We also need to specify the fishing effort or fishing mortality, which we set as an annual instantaneous fishing mortality rate. Now we can then plot the resulting fishing mortality. For illustration we will set a very high rate of 1.
...
We can now see how fishing affects the adult size spectrum **ASTA - not sure why you plot biomass density? I think number density is more intuitive, so I changed to that, but happy either way if there is a good reason**

```{r}
#we find a new equilibriums distribution of numbers at size, this time with fishing mortality imposed 
params_fishing <- singleSpeciesSteady(params_fishing)

bf_original <- plotSpectra(params, power = 1, wlim = c(10, NA), return_data = TRUE)

bf_original$Fishing <- "No Fishing" 
bf_fishing <- plotSpectra(params_fishing, power = 1, wlim = c(10, NA),return_data = TRUE)
bf_fishing$Fishing <- "Knife-edge"
bf <- rbind(bf_original, bf_fishing)

ggplot(bf, aes(x = w, y = value, colour = Legend, linetype = Fishing)) +
    geom_line() +
    scale_x_log10("Weight [g]") +
    scale_y_log10("Number density") + 
    # The following uses the same colours as the built-in plot functions
    scale_colour_manual(values = getColours(params), limits = force)
```

The difference in numbers may not seem to be very big, but note a very large range on the y axis. In reality the difference of numbers at size is almost ten-fold. The result is hardly surprising.

# Summary

1.  Species abundance in the ecosystem will strongly depend on the prey sizes that it feeds on. Because there are lots of small things and not so many large things, if a predator feeds on very small prey, then it will have a relatively more prey. This feeding preference is determined by the predation kernel, with a very important parameter - predator prey mass ratio or beta setting up the midpoint of the kernel. The width of the predation kernel depends on the sigma parameter. Wider kernels will increase the availability of food across broader size ranges and will affect the entire ecosystem. So beta and sigma are among the most important species parameters.
2.  The search rate of a species will determine how much food it can find. In mizer the exponent of how search rate scales with body size is $q$ and it is by default set as $q=2-\lambda+n$. The exponent of the maximum intake is usually assumed to scale with body size with as $n = 3/4$. This allows the model to reproduce emergent abundances at size that are consistent with the observed power laws.
3.  The feeding level (or satiation level) of fish will depend on the parameters for maximum intake rate and search volume and on the availability of suitably sized food. If we know something about the feeding level we expect in our modelled system, it will help us determine species feeding parameters $\gamma_0$ and $h$. Of course, like with many other things, mizer can set default values based on general size theory expectations.
4.  Species abundance will strongly depend on mortality and we will explore these aspects in greater detail in the next tutorials.
