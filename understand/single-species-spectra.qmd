...
If you get error messages saying that a particular package is not available, you will need to install that package. This was discussed in the preparatory tutorial on [installing tools](../prepare/install-tools.qmd#r-packages).
...
In this tutorial we will only be interested in the shape of the size spectrum, i.e., with how the total number of individuals is distributed over the different sizes. We are not yet interested in the overall abundance of the species. The overall abundance will be influenced by the total rate of reproduction for the species. This in turn will be influenced by the size spectrum, because that determines how many of the individuals are mature. This dynamic feedback will of course become important when we build real-world models in week 2 and use them in week 3. But for now we have enough on our plate without thinking also about this, so we will set the rate of reproduction to stay constant at the current level. In our single-species model. The following code gives you a first glimpse of how mizer provides functions to change aspects of a model after it has been created. But don't worry about the details for now.

```{r}
species_params(params)$constant_reproduction <- getRDD(params)
