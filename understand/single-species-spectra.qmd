...
The function returns a `MizerParams` object and we have assigned that to the variable `params`. We will be explaining more about this model as we go along, but you can also explore all the values and details that are stored in this `params` object, by typing params\@ in the console and selecting different options.

## Steady state spectrum

As a final simplification in this and the next two tutorials, we will only consider the steady state size spectra. The steady state is the state where in each size class the inflow of individuals through growth exactly balances the outflow of individuals through growth and death. This means that the size spectrum is in equilibrium. You might have heard that models are often "initialised" from some initial species abundances and then ran to equilibrium until model properties (species biomasses and size distributions) stop changing. However, we can calculate what sort of initial abundance we need to ensure that a species is in an equilibrium and this is what mizer does. The initial abundances in the `params` object that we created with `newSingleSpeciesParams()` have already been set to the steady state abundances.
...
Remember what these size spectra plots indicate? They show numbers or number density of individuals in various size bins. The initial slope of the taget species number density is negative, which of course means that there are fewer larger fish than smaller fish. That is of course understandable: lots of fish die while they are growing up, so there tend to be fewer fish in larger size classes. However, please bear in mind that you are now looking at numbers of individuals across size groups and not age groups. For people coming from age or stage structured modelling background it takes a while to understand that numbers in size bins do not represent age. Depending on how quickly individuals grow they will spend different times in each size bins - small individuals grow fast and move from one bin to another very quickly and large individuals may stay in the same bin for a long time.
...
ggplot(melt(cdf), aes(x = w, y = value)) +
    geom_line() + xlab("weight [g]") + ylab("% of total biomass")
```
...
Because this energy needs to be supplied by consumption of food, it is natural to assume that also the consumption rate scales allometrically with a power of $n = 3/4$. **\[ASTA - should we be more specific here? mizer has two values that determine intake - search rate and maximum intake rate. and two scaling parameters for them. I know it may be confusing, but for people who know mizer it will also not be clear which consumption we are talking about\]** Note that this is only the scaling of metabolism and feeding with size. At each given size, unless fish is almost starving, energy intake from feeding must be larger than energy use for metabolism (or maintenance), otherwise fish cannot grow and reproduce. Since intake and maintenance scales with $3/4$ then growth is also expected to scale with body weight with $3/4$. The $3/4$ are standard choices for the allometric exponents and are based on [metabolic theory of ecology](https://esajournals.onlinelibrary.wiley.com/doi/full/10.1890/03-9000), mizer also allows you to choose other values for these body size scaling exponents $p$ and $n$ (and there is a lot of debate in the literature about the correct values to use).

Finally, the death rate of organisms tends to scale allometrically with a power of $n - 1 = 3/4 - 1 = -1/4$. This means that the death rate experienced by larger individuals is smaller than that of small individuals (until some very final size, but mizer does not currently use senescence mortality). **ASTA - it is not clear whether this death rate is an emergent property or a mizer parameters. Do you mean background mortality?**
...
(because we had seen that the 61st size class starts at 1 gram). This is the instantaneous per-capita growth rate, measured in grams per year. Note that in mizer all rates are measured in units of year, but for many people daily values are easier to understand. Since growth rate here is an instantaneous rate we can simply divide it by 365 to get a daily rate (although note that mizer does not simulate processes on daily time steps)

```{r}
growth_rate[1, 61]/365
```

which gives us about 0.022g of growth per day for a 1g sized fish.
...
Of course in a real model, the growth rate would not so exactly follow a power law, because growth rate would very depending on food availability, for example. In this example food is the background resource and its abundance is fixed, so food abundance does not vary.
...
I won't do the maths here with you (and you probably don't want me to anyway), but we can see that the result makes sense. It tells us that the number density drops of faster with size if the mortality rate $\mu_0$ is higher or if the growth rate $g_0$ is smaller, which is what we would expect.
...
```{r}
#first let's get the maturity data (you can look at this proportion yourself)
mature <- maturity(params)
#now we get proportion of income used for reproduction 
reprod <- repro_prop(params)
#the final amount of energy going to reproduction at a given size will be a product of these two values
reprod_propotion <- mature * reprod
#we will convert it for ggplot with melt()
psi <- melt(reprod_propotion)
...
maturation_weight50 <- params@species_params$w_mat
maturation_weight25 <- params@species_params$w_mat25

p + 
  geom_vline(xintercept = maturation_weight50, lty = 2) +
...
ggplot(psi_changed_maturity) +
  geom_line(aes(x = w, y = value)) +
  geom_vline(xintercept = params_changed_maturity@species_params$w_mat, lty = 2) +
  geom_vline(xintercept = params_changed_maturity@species_params$w_mat25, lty = 2, col = "grey") + 
  labs(x = "Weight [g]",
       y = "Proportion invested into reproduction")
```
...
Next let us look at how our model, and more specifically its equilibrium conditions have changed. Now that maturity parameters have changed, the numbers at different sizes will also changes. So the steady state spectrum will also change. This means we first need to calculate the new steady state using the function singleSpeciesSteady(). This function will update numbers at size and other parameters, and we save it to the new params file
...
## ASTA - need to explain w(params)^2
#first we do .... THIS AND THAT (modify this)
...
1.  For a single species with a fixed resource and equilibrium (steady state) numbers at size will be determined by growth, mortality and investment into reproduction.
2.  Mizer stores and deals with number densities in size classes. To get the actual numbers at size we need to remember to multiple the number density (n array) by the width of the size class (dw). To get total biomass in each size class we further multiple this value by the weight of this size class (w)
3.  By default mizer assumes that intake **\[ASTA - decide which intake!\]** and metabolic rate scale with body size with the power of \$3/4\$ and death rate \[ASTA - parameter or emergent?\] scales with the power of $-1/4$. So we see that the juvenile size spectrum also scales with the power of $3/4$.
4.  The shape of the adult size spectrum will depend on the maturation parameters w_mat and w_mat25. By default mizer assumes a fairly steep reproduction allocation curve, which means that at maturation size energy flow rapidly gets diverted from growth to reproduction. But the user can make this curve less steep. Changes in maturation parameters will naturally affect the steady state and equilibrium numbers at size.
