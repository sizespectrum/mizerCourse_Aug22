...
As a final simplification in this tutorial and the next two, we will only consider the steady state size spectra. The steady state is the state where in each size class the inflow of individuals through growth exactly balances the outflow of individuals through growth and death. The initial abundances in the `params` object that we created with `newSingleSpeciesParams()` have already been set to the steady-state abundances.
...
I won't do the maths here with you (and you probably don't want me to anyway), but we can see that the result makes sense. It tells us that the number density drops of faster with size if the mortality rate is higher or if the growth rate is smaller, which is what we would expect.
...
The increase of abundance that we see at around the maturity size of our species is due to a drop in growth rate at that size. This in turn is due to the fact that the mature fish invests some of its energy income into reproduction. So the details of the shape of the adult spectrum is influenced by the details of how adults split their energy income between growth and reproduction.
...
Next let us look at how the steady state spectrum has changed. We first need to calculate the new steady state with
