...
If on uses a logarithmic weight axis, then the advantage of plotting the number density in log weight is that the area under the density curve is the total number of individuals in that size range. Similarly the area under the plot of the biomass density in log weight is the total biomass in that size range.
...
4.  To avoid the dependence on bin sizes, we use densities, where the number of individuals or the biomass in each bin are divided by the width of the bin. We refer to these as the number density or the biomass density respectively.
...
6.  Community size spectra are approximately power laws. When displayed on log-log axes, they therefore look like straight lines. The slope of the number density is approximately -2, the slope of the biomass density, which is also the number density in log weight, is approximately -1, and the slope of the biomass density in log weight is approximately 0, i.e., the biomass density in log weight is approximately constant.

7.  The species size spectra are not approximately power laws and thus do not look like straight lines on log-log axes. The approximate power law only emerges when we add all the species spectra together.
