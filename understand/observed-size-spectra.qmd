...
You can always easily read more information about the functions we are using by clicking on the function name (**ASTA: clicking how and where??**). This will open the function's help page in a new browser window.
:::
...
where the coefficient $a$ and the exponent $b$ are species-specific parameters (we'll discuss how to find such species parameters in a [later tutorial](../build/find-species-parameters.qmd)). The reason we like to work with weight as the measure of a fish's size is that there are well-known allometric relationships between weight and physiological rates. For example, metabolic rate is generally expected to scale as $w^{3/4}$ and mortality is often scales as $w^{-1/4}$, as we will be discussing in the section on [allometric rates](single-species-spectra.qmd#allometric-rates) in the next tutorial.
...
We had decided that we wanted the breaks between the bins to increase with powers of 2. We first create the vector `log_breaks` with the exponents. The `seq()` function creates a vector of numbers starting at `from` and going up to `to` in steps of size `by`. You do not need to give the names of the arguments because they can also be identified by their position. So you could also have written `seq(0, 11, 1)`. Such integer sequences are used so often that there is the even shorter notation `0:11` giving the same sequence.

The second line above creates a vector containing the powers of 2 with the exponents we just created in the vector `log_breaks`.
...
Now we can use these logarithmically increasing bins in the histogram:
...
You must then clone this to your computer and open it in RStudio. To do that you will need to follow the same steps as in the prepare week, when you conducted your first exercised. This means that you will start a new R project, checkout the week 1 repository using the GitHub url link with your name (e.g. https://github.com/Mizer-course/week-1-understand-astaaudzi) and then pull all the exercises for the first week. This may seem challenging, but it only requires a few clicks. You can follow the instructions from [here](../prepare/use-git-and-github.qmd)

In that worksheet you will find the following first exercise:
...
To understand better what the histogram did and to improve the plots further, we bin the data ourselves. We do this by first adding a bin number to each observation, which indicates in which bin each weight of the fish lies.
...
```{r eval=FALSE}
data_with_bins <- mutate(data, bin = cut(weight, breaks = binbreaks, right = FALSE, labels = FALSE))
```
...
binned_numbers <- binned_numbers |> 
    mutate(mid_points = mid_points[-length(mid_points)])

ggplot(binned_numbers) +
    geom_line(aes(x = mid_points, y = Number_dens)) +
    labs(x = "Weight [g]",
         y = "Number density")
```
...
We first calculate the midpoint of each bin using the same breaks as earlier. The last midpoint is after the last bin so we remove the value before adding the midpoints to the data frame.
:::
...
ggplot(binned_numbers) +
    ...
    scale_x_log10() + 
    scale_y_log10() +
    labs(x = "Log10 Weight [g]",
         y = "Log10 Number density")
```

Congratulations! You have plotted a size spectrum.

It is time for your second exercise of the course. Go back to your week 1 exercise project in RStudio, and do Exercise 2 in the worksheet 1 (**ASTA - as I understand it is not ready yet**):
...
```{r}
(model <- lm(log(Number_dens) ~ log(mid_points), data = binned_numbers))
```

This tells us that the straight-line approximation has a slope of about `r signif(model$coefficients[[2]], 3)`. We can also ask ggplot to put this line into the plot, together with its 95% (**ASTA - I asume this was 95% not 5%?? - also the model coefficient above**) confidence interval:

```{r warning=FALSE}
ggplot(binned_numbers, aes(x = mid_points, y = Number_dens)) +
    geom_line() + 
    scale_x_log10() + 
    scale_y_log10() +
    labs(x = "Weight [g]",
...
```{r}
(model <- lm(log(Biomass_dens) ~ log(mid_points), data = binned_biomass))
```

The slope is `r signif(model$coefficients[[2]], 4)` for the biomass density whereas it was `r exponent` for the number density. This makes perfect sense, because if we denote the biomass density by $B(w)$ then $B(w) = w N(w)$. Therefore if numbers at weight scale with the exponent $\lambda$ as $N(w) \propto w^{-\lambda}$ then biomass at weight will scale as $B(w)\propto w^{1-\lambda}$.

## A few final points before the summary: 
...
Above we calculated densities by dividing the total number or total biomass in each bin by the width that the bin has on the linear weight ($w$) axis. That is fine. Then we decided to use a logarithmic axis that showed $\log(w)$. But then would it not have been more natural to calculate the densities by dividing by the width of the bin on that logarithmic axis? **ASTA - I can't follow this and we need some example here. Log_bin_width is just 1 1 1 1 1 1. So what's the point of dividing by it?**
...
ggplot(binned_biomass) +
    geom_line(aes(x = mid_points, y = Biomass_dens),
              colour = "black") +
    geom_line(aes(x = mid_points, y = Sheldon_dens),
              colour = "blue") +
    scale_x_log10() +
    scale_y_log10() +
    labs(x = "Log Weight",
...
**Asta - suggest removing this** \[\[Don't worry about the fact that in places the estimate of the number density for the entire community is below the estimate of the number density for individual species. That just shows that we are dealing only with **estimates** of the true densities based on a finite amount of data.\]\]
...
::: {.callout-caution collapse="false"}
### Exercise 4 - ASTA: suggest removing this
...
5.  When we work with a logarithmic weight axis, then it is natural to use densities in log weight, where the numbers of individuals in each bin are divided by the log (**ASTA - what log? log 10, natural log or log 2?)** of the width of the bin. We refer to these as the number density in log weight or the biomass density in log weight respectively.
