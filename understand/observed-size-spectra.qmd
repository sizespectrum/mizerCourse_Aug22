...
p2 <- ggplot(data) +
    geom_histogram(aes(weight), fill = "blue", colour = "black",
                   breaks = binbreaks) +
    scale_x_continuous(name = "Weight [g]") +
    scale_y_log10(name = "Number of fish")
p2
```
...
This is the way in which Sheldon plotted the data and observed that size spectrum is approximately flat.
...
Below we plot the number densities (in black) and the biomass densities (in black), with the solid lines the densities in weight and the dashed lines the densities in log weight, all on the same axes.
