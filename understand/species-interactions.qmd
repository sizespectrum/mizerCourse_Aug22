...
Now, check how the diets of other species look like. Start from species 1 (smallest one) and all the way to species 11. By looking at the resulting graph, determine which of the following statements is true. Note that in the statements below we refer to the species as "lower" or "higher" based on their number, i.e., species "3" we say is lower than species "4"

1.  All species each all other species.

    FALSE. There are no higher species in diets of lower species **ASTA: these answers and comments should be moved into the worksheet I suppose.**
...
3.  Proportion of different species in the diets of higher species increases and then decreases. This is because as predators grow larger they prefer to eat larger prey.

    TRUE

4.  Proportion of different species in diets of higher species increases and then decreases. This is because as predators grow larger they start eating species that are the most abundant.

    FALSE

5.  Throughout its lifetime even the higher species (species 8-10) feed almost entirely on the resource.

    FALSE (most small size classes feed on the resource, but a species spends majority of time in the larger size classes, even if we don't have a lot of them in the model)
...
7.  Lower species can eat higher species.
    ...
    TRUE. Large individuals of small species can and will eat small individuals of large species. So what is prey and what is predator in marine ecosystems very much depends on size.
...
Now we might want to reduce the availability of resource to some predators. Perhaps we know that certain species are much more likely to feed on other fish rather than on similar sized plankton. Let us look at an example where species 9 has a 50% reduction in its interaction with resource.
...
TODO: Talk more about the trophic cascades. People won't necessarily know what that term means. **ASTA - not sure we need to talk much here, as Ken also introduced this concept in this talk. Perhaps just refer to his lecture**
...
mp_fishing_sss_steady <- projectToSteady(mp_fishing_sss)

plotSpectra2(mp_fishing_sss_steady, "less resource", mp, "original", 
            total = TRUE, power = 1, ylim = c(1e-8, NA), wlim = c(1e-3, NA))
```
...
**ASTA - I seem to be using abundance and biomasses interchangeably. We might need to check and correct perhaps? Or are we happy with that**
...
6.  Fishing does not only create undulations and trophic cascades in the fish spectra, but also in the resource spectrum.
