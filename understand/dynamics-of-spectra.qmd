...
We already discussed the [investment into reproduction](single-species-spectra.html#investment-into-reproduction) in an earlier tutorial. As mature individuals move from one size class to another they invest an increasing proportion of their income into reproduction and at their asymptotic size they would be investing all income into reproduction. Summing up all these investments from mature individuals of a particular species gives the total rate $E_R$ at which that species invests energy into reproduction. This total rate of investment is multiplied by a reproduction efficiency factor `erepro`, divided by a factor of 2 to take into account that only females reproduce, and then divided by the egg weight `w_min` to convert it into the rate at which eggs are produced. The `erepro` parameter or reproduction efficiency can vary between 0 and 1 (although 0 would be bad) and reflects the fact that only a fraction of energy invested into reproduction makes into viable eggs or larvae. The equation is: $$
R_{di} = \frac{\rm{erepro}}{2 w_{min}} E_R.
$$Once the reproduction efficiency factor is applied we get the rate of potential egg production which directly or linearly depends on the biomass of reproducing fish. Even though we scale the reproduction down by some factor, it is always the same scalar. So more reproducing fish will lead to more eggs. This means that production of smallest size individuals linearly depends on the energy invested by the entire fish populations and is not limited by their total amount or total biomass. In other words this is density-independent reproduction (RDI) (even though it is still dependent on density indirectly, see below). In mizer it is calculated with a function `getRDI()`. You will learn more about reproduction scaling in the future tutorials.
...
However there are other sources of density dependence that are not explicitly modelled mechanistically in mizer. An example would be a limited carrying capacity of suitable spawning grounds and other spatial effects. So mizer has another species parameter $R_{max}$ that gives the maximum possible rate of recruitment. Imposing a finite maximum reproduction rate leads to a non-linear relationship between energy invested and eggs hatched. This density-dependent reproduction rate \eqn{R_{dd}}{R_dd} is given by a Beverton-Holt type function: $$ R_{dd} = R_{di} \frac{R_{max}}{R_{di} + R_{max}}$$

Rather than looking at the formula, let's look at a figure:

```{r}
#| echo: false
#One species with high reproduction efficiency (erepro) and low carrying capacity (R_max)
erepro <- 0.7
R_max <- 0.4
E_R <- seq(0, 5, by = 0.05)
R_di = erepro * E_R
...
#Another species with lower reproduction efficiency and higher carrying capacity
erepro <- 0.3
R_max <- 0.7
R_di = erepro * E_R
R_dd <- R_di * R_max / (R_di + R_max)
...
ggplot(rbind(df, df2)) +
    ...
    scale_x_continuous(name = "Species biomass and energy invested [g/year]",
...
This figure shows two graphs of $R_{dd}$ (solid lines), one for higher $R_{max}$ (black) and one for lower $R_{max}$ (blue). The values of $R_{max}$ are indicated by the dotted lines. The dashed lines show the density-independent rate $R_{di}$. It is clear that the graph for $R_{dd}$ becomes more non-linear as $R_{max}$ gets smaller. The reproduction rate for the black solid line can still increase a lot if species biomass and total energy invested into reproduction keeps increasing. Instead the blue species is close to it maximum reproduction level and it reaches it quickly because it's `erepro` value is high. Such a species will recover quickly from depletion by fishing, for example.

This final rate of reproduction is calculated in mizer with `getRDD()`:
...
The ratio $R_{max} / R_{dd}$ we denote as the **reproduction level**. This name may remind you of the feeding level, which was the ratio between the maximum feeding rate and the actual feeding rate and described the level of density dependence coming from satiation.
...
Notice how the species have settled down to a new steady state after 30 years without any extinctions and the impact on species 1 is almost negligible. With higher reproduction level species are much more resilient to perturbations, because even a small number of fish can produce a maximum recruitment level. So the abundance of new larvae is quite stable, which also stabilises the model dynamics.
...
4\) Instead of a stock-recruitment relationship as used in other fisheries models, a mizer model relates the energy invested into reproduction to the number of eggs produced. The growth and mortality that the larvae experience until they are recruited to the fishery lead to density-dependence in the recruitment. Yet, some or a lot density dependence also applies to the egg production.

5\) The relation between energy invested in reproduction and the actual birth rate is described by two parameters: the density independent reproduction efficiency `erepro` and the maximum birth rate `R_max` which sets the maximum number of eggs that can recruit, independent of the total energy invested into reproduction.
