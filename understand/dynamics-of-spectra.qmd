...
$$Once the reproduction efficiency factor is applied we get the rate of potential egg production which directly or linearly depends on the biomass of reproducing fish. Even though we scale the reproduction down by some factor, it is always the same scalar. So more reproducing fish will lead to more eggs. So it is the amount of reproduction which is independent of total biomass, or in other words, density-independent reproduction (RDI) (even though it is still dependent on density indirectly, see below). In mizer it is calculated with a function `getRDI()`. You will learn more about reproduction scaling in the future tutorials.
...
6\) In practice a more useful way to parametrise the reproduction is by two other parameters: the birth rate $R_{dd}$ at steady state (which determines the total abundance of a species) and the *reproduction level* (which determines the amount of additional density dependence).
