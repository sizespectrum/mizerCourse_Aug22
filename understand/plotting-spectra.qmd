...
This tells us that the straight-line approximation has a slope of about $-2.241$. We can also ask ggplot to put this line, together with its 5% confidence interval:
...
If we denote the number density at weight $w$ by $N(w)$, then the above tells us that $$\log(N(w)) \approx 14.695 + \log(w)^{-2.241}.$$

If we exponentiate both sides to get rid of the logarithms this gives

$$ N(w) \approx \exp(14.695) w^{-2.241} = N(1) w^{-\lambda} $$
with $\lambda \approx -2.241$.
...
You will notice the reduced slope. In fact the slope of the Sheldon density is 1 less than the slope of the biomass density. If we denote the Sheldon density as $S(w)$ then we have the following relation to the biomass density $B(w)$ and the number density $N(w)$:

$$S(w) = w B(w) = w^2 N(w).$$

Sheldon's observation was that $S(w)$ is approximately constant over a large range of sizes $w$ from bacteria to whales. That corresponds to the earlier phrasing of Sheldon's observation that $N(w)\propto w^{-\lambda}$ with $\lambda$ close to 2.



## Kernel density estimate

Binning the data is not the only way to approximate the densities. One can also use the kernel density estimation method. ggplot2 even has that built in to its `geom_density()`. Here is a plot of the biomass density as estimated by the kernel density method:
